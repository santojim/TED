...
<html>
    <head>
        ...
        if(session.getAttribute("seller").equals("0") && session.getAttribute("buyer").equals("0") ){
            ...
            m_list=sqlDB.BringOutgoingMessages(u_id);
...
<!--################################################################################################################## -->
<!--##############################  PANEL PROBOLHS E3ERXOMENWN MNM  ################################################## -->
<!--################################################################################################################## -->
    <div class="container well well-lg " id="content_perfect">    
    <h2 style="text-align:center;">Απεσταλμένα</h2>
    ...
    <tbody>
            <%  int i;
                ...
                for(i=0;i<m_list.size();i++){
                    ...
                    out.println("<tr>");
                    out.print("<td>"+temp_message.getReceiveName()+ "</td>");
                    out.print("<td>"+temp_message.getMessage()+"</td>");
                    out.print("<td>"+temp_message.getTime()+"</td>");
                    out.print("<td><a href='deleteMessage.jsp?&page=out&id="+temp_message.getMessageId()+"'type='button' class='btn btn-danger' onclick='confirmation();'>Delete</a></td>");
